...
if (isset($_POST['changename'])) {
  if (isset($_POST['newusername'])){
    ...
    if ($check != 0){
      die("That username already exists! Try another!   <a href='modify.php'>Back</a>");
    }
    if (!ctype_alnum($newusername)) {
      die("Username contains special characters only letters and numbers are allowed!   <a href='modify.php'>Back</a>");
    }                                                                                                                
    if (strlen($newusername) > 20){
    die("Username must not contain more than 20 chatacters!   <a href='modify.php'>Back</a>");
    }
    $conn->query("UPDATE `users` SET `Username`='{$newusername}' WHERE `ID`='{$UID}'");
    die("You may now log in with your new username   <a href='login.php'>Login</a>"); 
